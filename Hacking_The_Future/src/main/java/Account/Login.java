...
public class Login {
    ...
    public User lgin() {
        ...
        User user = new User();
        try (
                Connection connect = DriverManager.getConnection(url, "root", pass); Statement statement = connect.createStatement()) { // connecting to the database
            ...
            if (emailExist) {
                if (passwordExist) {
                    System.out.println("\nWelcome back " + user.getUsername().toUpperCase() + "!\n");
                    return user;
